Shader "Custom/AAA_UltraRealSky"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // 6-Oktavlý Geliþmiþ Fraktal Gürültü (AllSky kalitesindeki yýrtýk bulut kenarlarý için)
            float fbm(float3 p) {
                float f = 0.0; float amp = 0.5;
                for(int i=0; i<6; i++){
                    f += amp * noise(p);
                    // Her adýmda uzayý döndürerek organik (fayanslaþmayan) bir kaos yarat
                    p = p * 2.03;
            ...
            fixed4 frag (v0 i) : SV_Target
            {
                ...
                // 1. ZEMÝN RENGÝ (Rayleigh Daðýlýmý Mantýðý)
                ...
                // Yýldýzlarýn parlamasýna zaman bazlý bir sinüs dalgasý (twinkle) eklendi
                ...
                // Güneþin etrafýndaki yumuþak korona/parlama
                float sunCorona = smoothstep(1.0 - _SunSize - _SunBlur, 1.0 - _SunSize, sunDot) * 0.5;
                // Gün batýmýnda tüm ufka yayýlan sýcak hüzme
                ...
                // 4. HACÝMLÝ, EVRÝMLEÞEN VE IÞIK KIRILMALI BULUTLAR
                if (dir.y > 0.0) 
                {
                    float2 cloudUV = dir.xz / (dir.y + 0.12); // Derinlik katar
                    cloudUV *= _CloudScale;
                    cloudUV.x += _Time.y * _CloudSpeed; // Rüzgar kaymasý
                    
                    // ÝÞTE SÝHÝR: Bulutlarýn 3D Gürültüsüne zamaný Y ekseninde ekliyoruz. Bu onlarý yavaþça eritip þekil deðiþtirtir.
                    ...
                    // Güneþ parlamasýný ekle (Sadece güneþli saatlerde)
